...
import { detectIssue } from '../services/geminiService';
...
export default function ReportIssue() {
    ...
    if (file && user) {
      try {
        ...
        reader.onloadend = async () => {
          ...
          startBackgroundUpload(compressedFile);

          try {
            const result = await detectIssue(base64, true);
            setDetection(result);
  ...
  const startBackgroundUpload = async (file: File | Blob) => {
    if (!user) return;
    ...
    try {
      ...
      setUploadError("Failed to upload image. We will try again during submission.");
    } finally {
  ...
  const handleSubmit = async () => {
    ...
    (async () => {
      try {
        ...
        // If background upload hasn't finished, wait for it or try one last time
        if (!finalImageUrl && compressedBlob) {
          try {
            const storageRef = ref(storage, `reports/${Date.now()}_${user.uid}.jpg`);
            const snapshot = await uploadBytes(storageRef, compressedBlob);
            finalImageUrl = await getDownloadURL(snapshot.ref);
          } catch (uploadErr) {
            console.error("Background upload failed in final step:", uploadErr);
          }
        }
        ...
        {step === 4 && (
          ...
          >
            ...
            <div className="space-y-6 text-left">
              ...
              {isUploading && (
                <div className="p-4 bg-blue-50 border border-blue-100 rounded-2xl flex items-center gap-4 dark:bg-blue-900/10 dark:border-blue-900/20">
                  <Loader2 className="text-blue-600 animate-spin" size={20} />
                  <div>
                    <h4 className="font-bold text-blue-900 dark:text-blue-400 text-sm">Uploading Image...</h4>
                    <p className="text-xs text-blue-700 dark:text-blue-500 mt-1">
                      We're uploading your image in the background to save time.
                    </p>
                  </div>
                </div>
              )}
                ...
                <div className="p-4 bg-emerald-50 border border-emerald-100 rounded-2xl flex items-center gap-4 dark:bg-emerald-900/10 dark:border-emerald-900/20">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-emerald-700 dark:text-emerald-500 mt-1">
                      Image upload complete. Submission will be instant.
                    </p>
                  </div>
                </div>
              )}
              ...
              {uploadError && (
                <div className="p-4 bg-rose-50 border border-rose-100 rounded-2xl flex items-center gap-4 dark:bg-rose-900/10 dark:border-rose-900/20">
                  <AlertCircle className="text-rose-600" size={20} />
                  <div>
                    <h4 className="font-bold text-rose-900 dark:text-rose-400 text-sm">Upload Issue</h4>
                    <p className="text-xs text-rose-700 dark:text-rose-500 mt-1">
                      Background upload failed, but we'll try again when you submit.
                    </p>
                  </div>
                </div>
              )}
